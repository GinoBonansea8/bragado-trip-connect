import ApiStatus from './ApiStatus'

const COMPANIES = ['21900', 'Santorini Turismo', 'Chevallier', 'Trenes Argentinos']
const STOPS = ['Bragado', 'Retiro', 'Once']

function App() {
  return (
    <>
      <header>
        <span className="brand">Bragado Trip Connect</span>
        <span className="badge">Operators</span>
      </header>

      <main>
        <h1>Publish a departure</h1>
        <p>
          Publish your schedules and fares so travellers can find them alongside
          every other operator on the route.
        </p>

        <form className="panel">
          <div className="field">
            <label htmlFor="company">Operator</label>
            <select id="company" disabled>
              {COMPANIES.map(company => <option key={company}>{company}</option>)}
            </select>
          </div>

          <div className="row">
            <div className="field">
              <label htmlFor="origin">From</label>
              <select id="origin" defaultValue="Bragado" disabled>
                {STOPS.map(stop => <option key={stop}>{stop}</option>)}
              </select>
            </div>

            <div className="field">
              <label htmlFor="destination">To</label>
              <select id="destination" defaultValue="Retiro" disabled>
                {STOPS.map(stop => <option key={stop}>{stop}</option>)}
              </select>
            </div>
          </div>

          <div className="row">
            <div className="field">
              <label htmlFor="date">Date</label>
              <input id="date" type="date" disabled />
            </div>

            <div className="field">
              <label htmlFor="departure">Departs</label>
              <input id="departure" type="time" disabled />
            </div>

            <div className="field">
              <label htmlFor="arrival">Arrives (approx.)</label>
              <input id="arrival" type="time" disabled />
            </div>
          </div>

          <div className="field">
            <label htmlFor="price">Fare (ARS)</label>
            <input id="price" type="number" min="0" step="0.01" disabled />
          </div>

          <button type="submit" disabled>Publish departure</button>
        </form>

        <h2>Published departures</h2>
        <p className="placeholder">
          Departures you publish will be listed here.
        </p>

        <ApiStatus />
      </main>
    </>
  )
}

export default App
